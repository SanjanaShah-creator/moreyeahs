...
function createStackedPlanesGeometry(n: number, width: number, height: number, heightSeg: number) {
  const geo = new THREE.BufferGeometry();
  const numV = n * (heightSeg + 1) * 2;
  const numF = n * heightSeg * 2;
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= heightSeg; j++) {
      ...
      uvs.set([uvXOff, j / heightSeg + uvYOff, uvXOff + 1, j / heightSeg + uvYOff], ui);
      if (j < heightSeg) {
...
const DirLight: FC<{ position: [number, number, number]; color: string }> = ({ position, color }) => {
  ...
  useEffect(() => {
    ...
    cam.top = 24; cam.bottom = -24; cam.left = -24; cam.right = 24;
    (cam as unknown as { far: number }).far = 64;
...
gl_FragColor.rgb -= rn/15.*uNoiseIntensity;`,
        ...
        material: { fog: true },
        uniforms: {
          diffuse: new THREE.Color(...hexToNormalizedRGB('#030810')),
          time: { value: 0 },
          roughness: 0.3,
          metalness: 0.3,
          uSpeed: { value: speed },
...
export default function EtherealBeams(props: BeamsProps) {
  return (
