    ...
    <>
      ...
      {(() => {
        const filterMap: Record<string, string> = {
          '/solutions/data-science':        'data-science',
          '/solutions/cloud-infrastructure': 'cloud',
          '/solutions/microsoft-services':   'microsoft',
          '/solutions/salesforce-services':  'salesforce',
          '/solutions/web-app-development':  'web-application-development',
        };
        const filter = filterMap[data.solutionHref] ?? '';
        const csHref = filter ? `/case-studies?filter=${filter}` : '/case-studies';
        return (
            ...
            <div className="container" style={{ position: 'relative', zIndex: 2 }}>
              <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 40, flexWrap: 'wrap', gap: 16 }}>
                <div>
                  ...
                  <h2 style={{ fontSize: 'clamp(24px,3vw,38px)', fontWeight: 800, letterSpacing: '-0.03em', color: 'var(--fg)', lineHeight: 1.1 }}>
                ...
                >
                  View all {data.solution} case studies <ArrowRight size={13} strokeWidth={2} />
                </Link>
              </div>
                ...
                {wpLoading
                  ...
                  : wpCases.length > 0
                    ...
                    : data.caseStudies.map((cs, i) => (
                        <motion.div key={i} variants={SC(0)} whileHover={{ y: -6, boxShadow: `0 24px 60px ${BLUE}22`, transition: { duration: 0.3 } }}>
                          <div style={{ border: '1px solid var(--border)', background: 'var(--card-bg)', display: 'flex', flexDirection: 'column', height: '100%', borderRadius: 22, overflow: 'hidden' }}>
                            ...
                            <div style={{ padding: '26px 26px 24px', display: 'flex', flexDirection: 'column', flex: 1 }}>
                              <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.10em', textTransform: 'uppercase', color: cs.accent, padding: '4px 10px', borderRadius: 999, background: `${cs.accent}12`, border: `1px solid ${cs.accent}25`, width: 'fit-content', marginBottom: 12 }}>{cs.industry}</span>
